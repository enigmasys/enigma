...
@ComponentScan(
    basePackages = ["common", "common.services.ObservationServiceImpl", "common.services.PremonitionProcessServiceImpl", "common.services.auth.AuthService"],
)
class DownloadCmd(
...
//    var observerID:String? = null
    ...
    @CommandLine.Option(
        required = false,
        names = ["-m", "--metadata"],
        description = ["Download ONLY all metadata files (without the data files)"],
    )
    var peek = false
                ...
                obsIndex?.let {
                    ...
                    when (it) {
                        "-1" -> logger.info("No Observations for Process ID: $processID ")
                            ...
                            {
                                val resultData =
                                    ObservationDownloadServiceObj.getAllPeekObservations(
                                        processID = processID,
                                        version = "0",
...
//                            prettyJsonPrint(it)
                ...
                for (i in (startObsIndex.toInt()..endObsIndex!!.toInt())) {
                    ...
                    var result =
                        ObservationDownloadServiceObj.getObservationFilesV3(
                            processID,
                            i.toString(),
                            i.toString(),
                            expiresInMins,
...
//              println("TransferID: ${values.transferId}")
                    ...
                    values.transferId?.let {
                        while (notFound) {
                            var transferStatus =
                                ObservationDownloadServiceObj.getTransferStat(
                                    processID = processID,
...
//                    logger.info("No Observations for Process ID: $processID ")
    ...
    private fun saveMetadata(
        resultData: UploadObservationObject?,
        ObsIndex: String?,
        version: String = "0",
    ) {
        resultData?.let {
...
//            val displayName = tmpTaxonomyData[0].displayName
            ...
            if (tmpTags?.size!! > 0) {
                val tmpdata =
                    tmpTaxonomyData[0].taxonomyVersion!!.branch?.let { it1 ->
                        tmpTaxonomyData[0].taxonomyVersion!!.id?.let { it2 ->
                            TaxonomyServerClient().getHumanTags(
                                tmp_baseurl,
                                it2,
                                it1,
                                mapper.writeValueAsString(tmpTags),
                            )
                        }
                    }
                if (tmpdata != null) {
